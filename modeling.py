...
def my_model(args):
    ...
    #
    data = pd.read_csv(f"./prepro/{args.data}_{args.time}h.csv")
    data["Date"] =  [data["Date"].values[i][6:] + "/" + data["Date"].values[i][3:5] + "/" + data["Date"].values[i][0:2] for i in range(data.shape[0])]
    ...
    exp_btc_tl = setup(train, target = target_columns[0], session_id = 111, fold = 5, use_gpu = True, silent = True, verbose = False)
    print("")
    ...
    tuned_svm_ = tune_model(svm_, fold = 5 , optimize ="AUC",  search_library= "optuna",choose_better = True)
    tuned_xgboost_ = tune_model(xgboost_, fold = 5, optimize ="AUC",  search_library= "optuna",choose_better = True)
    tuned_rf_ = tune_model(rf_, fold = 5, optimize = "AUC", search_library= "optuna",choose_better = True)
    tuned_knn_ = tune_model(knn_, fold = 5, optimize = "AUC", search_library= "optuna",choose_better = True)
    tuned_mlp_ = tune_model(mlp_, fold = 5, optimize = "AUC", search_library= "scikit-learn",choose_better = True)
    tuned_catboost_ = tune_model(catboost_, fold = 5, optimize = "AUC", search_library= "optuna",choose_better = True)
    tuned_et_      = tune_model(et_, fold = 5, optimize = "AUC", search_library= "optuna",choose_better = True )
